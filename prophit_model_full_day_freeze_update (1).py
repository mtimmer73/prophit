...
def compute_projection(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Unified pitcher BAA field used by UI/risk logic.
    # For hitters, lower pitcher BAA allowed versus the hitter's batting side is a risk.
    batter_side_for_baa = str(row.get("bats", "")).upper()
    if batter_side_for_baa == "L":
        pitcher_baa_vs_hand = row.get("pitcher_baa_vs_lhb_season")
        pitcher_baa_vs_hand_career = row.get("pitcher_baa_vs_lhb_career")
    elif batter_side_for_baa == "R":
        pitcher_baa_vs_hand = row.get("pitcher_baa_vs_rhb_season")
        pitcher_baa_vs_hand_career = row.get("pitcher_baa_vs_rhb_career")
    else:
        # Switch/unknown: use the more conservative season split when both are present.
        l_baa = row.get("pitcher_baa_vs_lhb_season")
        r_baa = row.get("pitcher_baa_vs_rhb_season")
        vals = [v for v in [l_baa, r_baa] if v is not None and pd.notna(v)]
        pitcher_baa_vs_hand = min(vals) if vals else None
        l_career = row.get("pitcher_baa_vs_lhb_career")
        r_career = row.get("pitcher_baa_vs_rhb_career")
        career_vals = [v for v in [l_career, r_career] if v is not None and pd.notna(v)]
        pitcher_baa_vs_hand_career = min(career_vals) if career_vals else None

    if pitcher_baa_vs_hand is None or pd.isna(pitcher_baa_vs_hand):
        pitcher_baa_vs_hand = pitcher_baa_vs_hand_career

    row["pitcher_season_baa_allowed_vs_batter_hand"] = pitcher_baa_vs_hand
    row["pitcher_baa_vs_batter_hand"] = pitcher_baa_vs_hand
    ...
    # Positive notes: only add these when the specific metric is actually supportive.
    ...
    # Risk notes: anything that would appear red/cautionary in the drawer should be captured here.
    ...
    if pitcher_baa_vs_hand is not None and pd.notna(pitcher_baa_vs_hand):
        if pitcher_baa_vs_hand < 0.230:
            ...
            risks.append(f"Pitcher matchup caution: opposing pitcher allows only a {pitcher_baa_vs_hand:.3f} season BAA vs this batter hand")
        elif pitcher_baa_vs_hand >= 0.280:
    ...
    elif row["matchup_score"] >= 65:
        positives.append("Favorable matchup profile")

    if row.get("model_vs_market_avg_delta") is not None and pd.notna(row.get("model_vs_market_avg_delta")):
        if row["model_vs_market_avg_delta"] <= -0.025:
            risks.append(f"Model AVG is materially below season AVG by {row['model_vs_market_avg_delta']:.3f}")
        elif row["model_vs_market_avg_delta"] < -0.010:
            risks.append(f"Model AVG is slightly below season AVG by {row['model_vs_market_avg_delta']:.3f}")

    if row.get("model_avg") is not None and pd.notna(row.get("model_avg")) and row["model_avg"] < 0.260:
        risks.append(f"Model AVG is below preferred threshold at {row['model_avg']:.3f}")

    if row.get("matchup_score") is not None and pd.notna(row.get("matchup_score")):
        if row["matchup_score"] < 45:
            risks.append(f"Low matchup score ({row['matchup_score']:.0f})")
        elif row["matchup_score"] < 55:
            risks.append(f"Moderate matchup caution ({row['matchup_score']:.0f} matchup score)")

    if row.get("split_reliability_score") is not None and pd.notna(row.get("split_reliability_score")) and row["split_reliability_score"] < 40:
        risks.append(f"Low split reliability score ({row['split_reliability_score']:.0f})")

    if row["split_ab_3yr_vs_opp_hand"] < 75:
        risks.append("Real handedness split sample is smaller than preferred")

    if row.get("last15_hits_per_ab") is not None and pd.notna(row.get("last15_hits_per_ab")) and row["last15_hits_per_ab"] < 0.200:
        risks.append(f"Recent 15 AB form is cold at {row['last15_hits_per_ab']:.3f}")

    if row.get("last30_hits_per_ab") is not None and pd.notna(row.get("last30_hits_per_ab")) and row["last30_hits_per_ab"] < 0.225:
        risks.append(f"Recent 30 AB form is below target at {row['last30_hits_per_ab']:.3f}")
    ...
    # Risk tag should be true when any of the drawer risk logic flags a real issue.
    row["fade_risk_tag"] = (
        row["projected_ab"] < 3.3
        or row["confidence_score"] < 45
        or row["pinch_hit_risk"] > 0.50
        or row["data_quality_flag"] == "no_lineup_order"
        or len(risks) > 0
    )
