...
def compute_delta_avg(row: Dict[str, Any]) -> Optional[float]:
    """
    Delta AVG = model AVG minus season/market AVG.

    Preferred source is model_vs_market_avg_delta if the model snapshot already writes it.
    Fallback computes the value from model AVG and season/market AVG fields.
    """
    existing = safe_float(
        row.get("model_vs_market_avg_delta")
        or row.get("delta_avg")
        or row.get("avg_delta")
        or row.get("model_avg_delta")
        or row.get("model_vs_season_avg_delta")
    )
    if existing is not None:
        return round(existing, 3)

    model_avg = safe_float(
        row.get("model_avg")
        or row.get("final_hit_rate")
        or row.get("projected_avg")
    )

    season_avg = safe_float(
        row.get("season_market_avg")
        or row.get("season_hits_per_ab")
        or row.get("season_avg")
        or row.get("batter_season_avg")
        or row.get("avg_season")
    )

    if model_avg is None or season_avg is None:
        return None

    return round(model_avg - season_avg, 3)


def compute_fair_odds(row: Dict[str, Any]) -> Optional[int]:
    """
    Fair odds = American odds implied by 1+ hit probability.

    Preferred source is fair_odds_american if the model snapshot already writes it.
    Fallback computes American odds from one_plus_hit_probability.
    """
    existing = safe_int(
        row.get("fair_odds_american")
        or row.get("fair_odds")
        or row.get("model_fair_odds_american")
        or row.get("implied_fair_odds")
    )
    if existing is not None:
        return existing

    p = safe_float(row.get("one_plus_hit_probability"))
        ...
        try:
            ...
            result = {
                ...
                **actual,
                ...
                "model_vs_market_avg_delta": compute_delta_avg(row),
                "fair_odds_american": compute_fair_odds(row),
...
def validation_pitcher_career_baa_vs_batter_hand(item: Dict[str, Any]) -> Any:
    bats = str(item.get("bats") or "").upper()
    if bats == "L":
        return (
            item.get("pitcher_baa_vs_lhb_career")
            or item.get("opp_pitcher_baa_vs_lhb_career")
            or item.get("opp_pitcher_avg_allowed_vs_lhb_career")
        )
    if bats == "R":
        return (
            item.get("pitcher_baa_vs_rhb_career")
            or item.get("opp_pitcher_baa_vs_rhb_career")
            or item.get("opp_pitcher_avg_allowed_vs_rhb_career")
        )
    return None


def validation_pitcher_season_baa_vs_batter_hand(item: Dict[str, Any]) -> Any:
    bats = str(item.get("bats") or "").upper()
    if item.get("pitcher_season_baa_allowed_vs_batter_hand") is not None:
        return item.get("pitcher_season_baa_allowed_vs_batter_hand")
    if item.get("pitcher_baa_vs_batter_hand") is not None:
        return item.get("pitcher_baa_vs_batter_hand")
    if bats == "L":
        return (
            item.get("pitcher_baa_vs_lhb_season")
            or item.get("opp_pitcher_baa_vs_lhb_season")
            or item.get("opp_pitcher_avg_allowed_vs_lhb_season")
        )
    if bats == "R":
        return (
            item.get("pitcher_baa_vs_rhb_season")
            or item.get("opp_pitcher_baa_vs_rhb_season")
            or item.get("opp_pitcher_avg_allowed_vs_rhb_season")
        )
    return None
...
def write_validation_rows_from_ledger() -> None:
    ...
    for item in ledger.values():
        ...
        rows.append({
            ...
            "model_vs_market_avg_delta": compute_delta_avg(item),
            "fair_odds_american": compute_fair_odds(item),
            "pitcher_season_baa_allowed_vs_batter_hand": validation_pitcher_season_baa_vs_batter_hand(item),
            "pitcher_career_baa_allowed_vs_batter_hand": validation_pitcher_career_baa_vs_batter_hand(item),
            "pitcher_baa_vs_batter_hand": item.get("pitcher_baa_vs_batter_hand"),
            "pitcher_baa_vs_lhb_season": item.get("pitcher_baa_vs_lhb_season"),
            "pitcher_baa_vs_rhb_season": item.get("pitcher_baa_vs_rhb_season"),
            "pitcher_baa_vs_lhb_career": item.get("pitcher_baa_vs_lhb_career"),
            "pitcher_baa_vs_rhb_career": item.get("pitcher_baa_vs_rhb_career"),
            "opp_pitcher_baa_vs_lhb_season": item.get("opp_pitcher_baa_vs_lhb_season"),
            "opp_pitcher_baa_vs_rhb_season": item.get("opp_pitcher_baa_vs_rhb_season"),
            "opp_pitcher_baa_vs_lhb_career": item.get("opp_pitcher_baa_vs_lhb_career"),
            "opp_pitcher_baa_vs_rhb_career": item.get("opp_pitcher_baa_vs_rhb_career"),
            "opp_pitcher_avg_allowed_vs_lhb_season": item.get("opp_pitcher_avg_allowed_vs_lhb_season"),
            "opp_pitcher_avg_allowed_vs_rhb_season": item.get("opp_pitcher_avg_allowed_vs_rhb_season"),
            "opp_pitcher_avg_allowed_vs_lhb_career": item.get("opp_pitcher_avg_allowed_vs_lhb_career"),
            "opp_pitcher_avg_allowed_vs_rhb_career": item.get("opp_pitcher_avg_allowed_vs_rhb_career"),
            "opp_pitcher_hand": item.get("opp_pitcher_hand"),
            "bats": item.get("bats"),
            "season_avg_vs_pitcher_hand": item.get("season_avg_vs_pitcher_hand"),
            "split_hits_per_ab_season_vs_opp_hand": item.get("split_hits_per_ab_season_vs_opp_hand"),
            "split_hits_per_ab_3yr_vs_opp_hand": item.get("split_hits_per_ab_3yr_vs_opp_hand"),
            "split_ab_3yr_vs_opp_hand": item.get("split_ab_3yr_vs_opp_hand"),
            "real_avg_vs_rhp_career": item.get("real_avg_vs_rhp_career"),
            "real_avg_vs_lhp_career": item.get("real_avg_vs_lhp_career"),
            "alltime_avg_vs_rhp": item.get("alltime_avg_vs_rhp"),
            "alltime_avg_vs_lhp": item.get("alltime_avg_vs_lhp"),
            "last15_hits_per_ab": item.get("last15_hits_per_ab"),
            "last50_hits_per_ab": item.get("last50_hits_per_ab"),
            "last100_hits_per_ab": item.get("last100_hits_per_ab"),
            "last200_hits_per_ab": item.get("last200_hits_per_ab"),
            "last15ab_avg_vs_rhp": item.get("last15ab_avg_vs_rhp"),
            "last15ab_avg_vs_lhp": item.get("last15ab_avg_vs_lhp"),
            "last50ab_avg_vs_rhp": item.get("last50ab_avg_vs_rhp"),
            "last50ab_avg_vs_lhp": item.get("last50ab_avg_vs_lhp"),
            "last100ab_avg_vs_rhp": item.get("last100ab_avg_vs_rhp"),
            "last100ab_avg_vs_lhp": item.get("last100ab_avg_vs_lhp"),
            "last200ab_avg_vs_rhp": item.get("last200ab_avg_vs_rhp"),
            "last200ab_avg_vs_lhp": item.get("last200ab_avg_vs_lhp"),
    ...
    write_json(VALIDATION_ROWS_JSON, {
        "meta": {
            ...
            "row_count": len(rows),
            "purpose": "Row-level graded history for PropHit Validation Lab, including Delta AVG and Fair Odds filters.",
        },
        "rows": rows,
    })
...
def write_grade_outputs(target_date: str, graded_rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    preferred = ["date", "game_time_et", "game_pk", "team", "opp", "player", "player_id", "market", "graded_projection_run_id", "graded_projection_run_dt_et", "lineup_spot_num", "is_confirmed_starter", "projected_ab", "projected_hits", "one_plus_hit_probability", "model_vs_market_avg_delta", "fair_odds_american", "hit_score", "confidence_score", "best_bet_tier", "actual_hits", "actual_ab", "actual_total_bases", "one_plus_hit_win", "result", "projected_hits_error", "grade_status", "final_grade_key", "graded_at_et", "game_status", "_snapshot_file"]
    if not df.empty:
